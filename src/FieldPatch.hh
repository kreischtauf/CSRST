...
public:
    ...
    void gather(ParticleAttrib< Vector_t > & fld,
                const ParticleAttrib< Vector_t > & pp,
                const size_t & comp,
                const IntOp & op,
                const size_t & from,
                const size_t & to) const;
    template<class IntOp>
    void gather(ParticleAttrib< double > & fld,
                const ParticleAttrib< Vector_t > & pp,
                const IntOp & op,
                const size_t & from,
                const size_t & to) const;
    template<class IntOp>
    void scatter(const ParticleAttrib< Vector_t > & fld,
                 const ParticleAttrib< Vector_t > & pp,
                 const size_t & comp,
                 const IntOp & op);
    template<class IntOp>
    void scatter(const ParticleAttrib< double > & fld,
                 const ParticleAttrib< Vector_t > & pp,
                 const IntOp & op);
...
FieldPatch<T>::FieldPatch():
    _data(1,0.0)
{
    _gatherTimer = Timings::getTimer("field gather");
    _lDom[0] = Index(0, -1, -2);
    _lDom[1] = Index(0, -1, -2);
}
...
#include "ScatterGatherImpl.hh"

template<class T>
template<class IntOp>
void FieldPatch<T>::gather(ParticleAttrib< Vector_t > & fld,
                           ...
                           const size_t & to) const
{
    Timings::startTimer(_gatherTimer);
    ScatterGatherImpl<IntOp, T>::gather(*this, fld, R, comp, from, to);
    Timings::stopTimer(_gatherTimer);
}

template<class T>
template<class IntOp>
void FieldPatch<T>::gather(ParticleAttrib< double > & fld,
                           const ParticleAttrib< Vector_t > & R,
                           const IntOp & op,
                           const size_t & from,
                           const size_t & to) const
{
    Timings::startTimer(_gatherTimer);
    ScatterGatherImpl<IntOp, T>::gather(*this, fld, R, from, to);
    Timings::stopTimer(_gatherTimer);
}

template<class T>
template<class IntOp>
void FieldPatch<T>::scatter(const ParticleAttrib< Vector_t > & fld,
                            const ParticleAttrib< Vector_t > & R,
                            const size_t & comp,
                            const IntOp & op)
{
    ScatterGatherImpl<IntOp, T>::scatter(*this, fld, R, comp);
}

template<class T>
template<class IntOp>
void FieldPatch<T>::scatter(const ParticleAttrib< double > & fld,
                            const ParticleAttrib< Vector_t > & R,
                            const IntOp & op)
{
    ScatterGatherImpl<IntOp, T>::scatter(*this, fld, R);
}
